...
let serviceAccount = require("./ecom-website-88cb6-firebase-adminsdk-6oltd-d22398907b.json");
...
const bucketName = "ecom-website-petsmart";
...
app.post('/seller', (req, res) => {
    ...
    if (!name.length || !address.length || !about.length || number.length < 10 || !Number(number)) {
        ...
        return res.json({'alert': 'you must have to agree our terms and conditions'})
    } 
    else {
...
app.post('/add-product', (req, res) => {
    let { name, shortDes, des, image, sizes, actualPrice, discount, sellPrice, stock, tags, tac, email, draft, id } = req.body;
    ...
    if (!draft) {
        ...
        } else if (!image.length) { // image link array
...
app.post('/order', (req, res) => {
    ...
    const mailOption = {
        from: 'jainminod2203@gmail.com',
        to: email,
        subject: 'PetsMart: Order Placed',
        html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
    ...
    .then(data => {
        transporter.sendMail(mailOption, (err, info) => {
            if (err) {
                // res.json({'alert': 'oops! it seems like some error has occured in placing your order'});
